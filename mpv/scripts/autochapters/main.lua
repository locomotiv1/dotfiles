...
local db_url =
	"https://github.com/manami-project/anime-offline-database/releases/download/2025-32/anime-offline-database-minified.json"
...
local function guess(path, auto)
	...
	local guessit = mp.command_native({
		name = "subprocess",
		playback_only = false,
		capture_stdout = true,
		args = { "guessit", "-jE", "--", path },
	})
...
local function update_db()
	local anime_relations = mp.command_native({
		name = "subprocess",
		playback_only = false,
		capture_stdout = true,
		args = { "curl", "-s", "-L", relations_url },
	})
	...
	for rule in rules:gmatch("[^\r\n]+") do
		local in_mal_id, in_ep_start, in_ep_end, out_mal_id, out_ep_start, out_ep_end, self_redirect = string.match(
			rule,
			"^%- ([0-9]+)|[0-9?]+|[0-9?]+:([0-9]+)%-?([0-9?]*) %-> ([0-9~]+)|[0-9?~]+|[0-9?~]+:([0-9]+)%-?([0-9?]*)(!?)"
		)
		if in_mal_id then
			local out_mal_id_string = out_mal_id
			in_ep_start, in_ep_end, out_ep_start, self_redirect =
				tonumber(in_ep_start), tonumber(in_ep_end), tonumber(out_ep_start), self_redirect
			...
			table.insert(
				relations[in_mal_id],
				{ a = in_ep_start, b = in_ep_end, x = out_ep_start, z = out_mal_id, t = title }
			)
	...
	local manami = mp.command_native({
		name = "subprocess",
		playback_only = false,
		capture_stdout = true,
		args = { "curl", "-s", "-L", db_url },
	})
...
local function api_lookup(title, episode, duration, auto)
	...
	if not auto then
		mp.osd_message(
			"Searching for "
				.. (relation_title or title)
				.. (display_episode and (" Episode " .. display_episode) or "")
				.. "..."
		)
	end
	...
	local url = api_url
		...
		.. "/"
		.. (episode or 1)
		.. "?types[]=op&types[]=ed&types[]=mixed-op&types[]=mixed-ed&types[]=recap&episodeLength="
		.. duration
	local aniskip = mp.command_native({
		name = "subprocess",
		playback_only = false,
		capture_stdout = true,
		args = { "curl", "-s", url },
...
local function file_load()
	...
	if not options.on_file_load then
		return
	end
					...
					if not found then
						mp.osd_message(
							"No chapters found for "
								.. (title and (title .. (episode and (" Episode " .. episode) or "")) or media_title)
						)
					end
				end)
			else
