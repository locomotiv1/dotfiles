...
local overlay_timer = mp.add_periodic_timer(0.1, function()
    ...
    local detected_status = (correction_overlay.detected and correction_overlay.detected.current_status) or "watching"

    local focus = correction_overlay.focus
    local id_text = correction_overlay.id_input ~= "" and correction_overlay.id_input or "(paste MAL URL or type ID)"
...
local function extract_mal_id(input_text)
    ...
    -- Try to match MAL URL or raw ID
    return trimmed:match("myanimelist%.net/anime/(%d+)") or trimmed:match("^(%d+)$")
end
...
local function submit_correction()
    local mal_id = extract_mal_id(correction_overlay.id_input)
    if not mal_id then
        mp.osd_message("Invalid MAL URL/ID.", 2)
        return
    end
    ...
    local args = {python_command, correction_overlay.s_dir .. "malUpdater.py", correction_overlay.path, "correct", python_options_json, mal_id}
...
local function open_correction_overlay(path, s_dir, detected)
    ...
    mp.add_forced_key_binding("any_unicode", "correct_overlay_any_unicode", function(key)
        ...
        if key.event and key.event ~= "down" and key.event ~= "repeat" then return end
...
function M.correct_anime_id(get_path)
    ...
    mp.command_native_async({
        name = "subprocess",
        args = {python_command, s_dir .. "malUpdater.py", path, "info", python_options_json},
        capture_stdout = true
    }, function(success, result)
        local detected = nil
